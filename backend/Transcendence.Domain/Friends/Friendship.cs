...
#pragma warning restore CS8618
	...
	public Friendship (Guid userAId, Guid userBId, DateTime createdAt)
	{
		if (userAId == Guid.Empty)
			throw new ArgumentException("User ids cannot be empty.", nameof(userAId));
		if (userBId == Guid.Empty)
			throw new ArgumentException("User ids cannot be empty.", nameof(userBId));
		if (userAId == userBId) throw new CannotFriendYourselfException();

		var (u1, u2) = userAId.CompareTo(userBId) < 0 ? (userAId, userBId) : (userBId, userAId);// normalized pair - simmetrical friendship
		User1Id = u1;
		User2Id = u2;
		CreatedAt = createdAt;
	}
